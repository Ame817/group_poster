...
RNA_sequence=''
...
def frequent_codon(RNA_sequence):    
...
codon_dict=frequent_codon(RNA_sequence)

def frequent_amino_acids(RNA_sequence):
    aa_dict={}
    #count frequency
...
aa_dict=frequent_amino_acids(RNA_sequence)
def aminoacids_piechart(aa_dict): #generate pie chart, in decsending order.
    labels,values = aa_dict.keys(),aa_dict.values() 
    sort_list = sorted(zip(labels, values), key=lambda x: x[1], reverse=False) 
